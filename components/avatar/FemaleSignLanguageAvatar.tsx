...
import { prepareMixamoScene, normalizeMixamoBoneNames, openArmsOutward, type PrepareMixamoOptions } from './prepareMixamoAvatar'
...
interface FemaleAvatarProps {
  ...
  /** GLB Mixamo ♀ / Hellen / Harry. */
  modelUrl?: string
  /** Bake / détente épaules native. `null` = Harry (normalize + sol). */
  prepareOptions?: Pick<
    PrepareMixamoOptions,
    'bakeScale' | 'armOpen' | 'armForward' | 'shoulderDrop' | 'shoulderDepress'
  > | null
}

export default function FemaleSignLanguageAvatar({
  frames,
  isPlaying,
  fps = 25,
  onDone,
  ...
  const scene = useMemo(() => {
    try {
      // Hellen / Harry : ne pas passer par bakeUniformScale (déjà en mètres).
      if (prepareOptions == null || prepareOptions.bakeScale === false) {
        ...
        if (prepareOptions?.armOpen && prepareOptions.armOpen > 0) {
          openArmsOutward(root, prepareOptions.armOpen)
        }
        return root
      }
      return prepareMixamoScene(gltfScene, 1.7, {
        bakeScale: true,
        naturalArms: false,
        armOpen: prepareOptions.armOpen,
        armForward: prepareOptions.armForward,
        shoulderDrop: prepareOptions.shoulderDrop,
        shoulderDepress: prepareOptions.shoulderDepress,
      })
    } catch (err) {
      console.error('[avatar] prepare failed', modelUrl, err)
      return gltfScene
    }
  }, [gltfScene, modelUrl, prepareOptions])
  ...
  useEffect(() => {
    ...
    scene.traverse((node) => {
      ...
      if ((node as THREE.SkinnedMesh).isSkinnedMesh) {
        ...
        if (mesh.skeleton) mesh.bindMode = 'attached'
  ...
  const applyIdlePose = useCallback(() => {
    ...
    // 1. Pose idle déjà capturée → réutiliser
    ...
    // 3. Fallback : pose de repos du GLB
    for (const [name, bone] of Object.entries(bones.current)) {
  ...
  // dispose={null} : ne pas détruire les géométries partagées du cache GLTF au changement d’avatar
